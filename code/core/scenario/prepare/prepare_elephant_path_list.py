...
def prepare_elephant_path_list(args):
    # load topology
    tic = time.time()
    ...
    tic = time.time()
    print(f'[+] generating canonical path list of elephant flow')
    i = 0
    def job_fn(j):
        pl = util.get_path_list_elephant_flow(G, i, j, args)
        if j % 10 == 0:
            toc = time.time()
    ...
    # save the canonical path list of elephant flow
