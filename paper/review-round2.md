...
# 第三轮 · 5 条评审意见处置记录（2026-09）

## 1. 本轮意见与处置

| # | 意见 | 处置 | 状态 |
|---|---|---|---|
| 1 | 参考文献增加到 50 篇 | refs.bib 38→50，新增 12 条全部经 WebSearch 核实元数据；boyd2011、hong2013 原先未引用，已补挂正文 | 已完成 |
| 2 | 不同星座 ×3 + 流量矩阵生成方式 ×2 | 见下节可行性核查，本轮**未启动** | 待办 |
| 3 | Table 7 的 "supplementary CSV" 有问题，改柱状图 | 表格替换为 `figs/overall_bars.pdf`（mean±std 误差棒），删除悬空句；Fig.4 散点保留 | 已完成 |
| 4 | 图 2 的 node-level 无意义、正文未提 | 图 2 改为三面板（真值 / flow-level / node-level），§3 同步说明两种粒度；并补 node-level 实验（Phase D）以支撑引言的整行缺失论证 | 图已完成，实验进行中 |
| 5 | SpaTE 的 MLU 优势不稳定，需解释且结果应更有说服力 | §5.3 新增机理段落（定量）；另启动无后处理域主对比实验以隔离 allocator | 解释已完成，实验进行中 |

## 2. 意见 2 的可行性核查（决定推迟的依据）

核查对象：`code/data/topo/`、`code/data/traffic_matrix/`、`code/core/scenario/data/prepare_starlink_tm.py`。

**星座规模只有 2 种，不是 3 种。**

| 数据 | 卫星数 | 有向 ISL | 快照 |
|---|---|---|---|
| `starlink_22_24` | 528 | 2,112 | 93 |
| `starlink_22_72` | 1,584 | 6,336 | 101 |

`prepare_starlink_tm.py` 只负责打包 STK 导出的 `tmMatrix{i}.npy`，而 `code/data/raw/` 不存在。**第三种规模必须先用 STK 重新导出轨道数据**，无法从现有文件推导。
（`Starlink2272Drop5` 是 1,584 节点 / 6,020 边的 5% ISL 失效变体，属于同一规模下的**拓扑条件**，不是第三种星座规模。）

**流量矩阵生成方式确实有 2 种真实可用。**

| 生成方式 | 每快照活动对 | 每快照总量 | 非零均值 / 中位 |
|---|---|---|---|
| pop（当前使用） | 5,583 | 890,242 | 159.5 / 57.0 |
| gdp（`starlink_gdp_22_72.npz`，已存在） | 18,595 | 215,426 | 11.6 / 3.0 |

两者结构差异很大：gdp 是"多对、小流"，pop 是"少对、大流"。gdp 的 union pairs 约 125,949，是 pop 的 6,334 的约 20 倍，PathNode 规模约 504K，**首次路径生成与显存都有风险**（`run_final_matrix.sh` 注释已标注）。

**结论**：可做的是 **2 个真实规模 × 2 种真实 TM 生成方式**（可再叠加 Drop5 作为拓扑条件），达不到"星座 ×3"。要凑第三种规模需新增 STK 数据，属于新数据采集，不在本轮范围。

## 3. 遗留待办

- **T-1（意见 2）**：2 规模 × 2 TM 生成方式的交叉实验；启动前须先在单格上验证 gdp 的路径生成耗时与峰值显存，再决定并发。
- **T-2**：若要满足"星座 ×3"，需补 STK 轨道导出（例如 22×48），属新数据采集。
- **T-3**：摘要的 sub-0.4-s 推理延迟本轮未重测；历史记录提示该计时可能只覆盖 transform/repair/extract 三段，需重新界定口径后复测。

## 4. 本轮实验批次状态

| 批次 | 用途 | 协议差异 | 规模 | 进度 |
|---|---|---|---|---|
| Phase C `norepair_main` | 意见 5：隔离 allocator | `--admm-steps 0`（关后处理） | 5 ρ × 6 方法 × 5 seed = 150 | 72/150 |
| Phase D `nodeobs` | 意见 4：node-level 观测 | 保持部署协议 `--admm-steps 2 --repair-input nbr`，加 `--obs-type node` | 3 ρ × 3 方法 × 3 seed = 27 | 0/27 |

运行记录（须如实保留）：
- 首次以 6 并发启动时，GPU 上有一个无关任务占 12.13 GiB，导致 6 个 `ours-0.05` / `test-style-0.05-0` 格 **OOM**。OOM 与被中断的格均未写入 CSV（脚本按 `Testing: 100` 行提取结果，缺失即记 FAIL/OOM），无数据污染。已降并发重启，OOM 格自动重试。
- untrained 25 格与历史 `norepair_curve.csv` 交叉验证 14/15 精确一致；唯一差异为 ρ=0.3 seed1（2.6648 → 2.6690，0.16%），归因于远端工作副本自上次运行后存在未提交修改，**不宣称完全一致**。
