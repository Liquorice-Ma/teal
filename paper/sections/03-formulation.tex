...
ratios after a feasibility-restoration step that enforces
\eqref{eq:demand}--\eqref{eq:nonneg} against the true $D^t$ (realized by
ADMM refinement and rounding in \cref{sec:training}). Formally,
\begin{equation}
    \max_{\pi} \;
    ...
    \label{eq:sparse-obj}
\end{equation}
where the expectation is over traffic and topology dynamics. The problem
is a partially observable sequential decision problem: unlike
\eqref{eq:obj}--\eqref{eq:nonneg}, where the same $D^t$ appears in both
