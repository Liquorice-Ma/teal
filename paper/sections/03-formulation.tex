...
\begin{figure}[t]
    ...
    \caption{Sparse observation is structured, not random. \textbf{(a)} A
    ground-truth traffic matrix from the Starlink $22{\times}72$ trace (150
    busiest satellites shown); traffic concentrates on a few
    source-destination pairs. \textbf{(b)} Its node-level sparse observation:
    white cells carry no traffic, gray rows are unmeasured source satellites
    whose entire demand row is hidden, and colored cells are observed demands.
    Because metering is per satellite, one unmeasured node removes a whole row
    at once, so the controller must allocate demands it has never seen rather
    than fill in scattered missing entries.}
    \label{fig:sparse-tm}
\end{figure}
