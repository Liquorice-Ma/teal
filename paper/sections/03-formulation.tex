...
$r_{i,k} \ge 0$ and $\sum_k r_{i,k} = 1$, so every demand is fully served. Main notations are summarized in \cref{tab:notation}.
...
be fully served, so $\theta$ cannot be lowered by withholding traffic.
...
ratios after optional neighbor-assisted rebalancing. The ratios remain
conservation-feasible by \cref{eq:conserve}; the rebalancing only shifts
traffic among candidate paths to reduce residual overload. The sparse TE
objective is
\begin{equation}
    \min_{\pi} \;
...
consumes real capacity even though the policy could not see it, so
misjudging it inflates realized MLU. The problem
