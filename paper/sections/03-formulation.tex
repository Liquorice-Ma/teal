...
\end{align}
where \eqref{eq:conserve} enforces flow conservation: every demand must
...
epoch~\cite{sate}, and --- more fundamentally for this paper --- the
link-load terms in \eqref{eq:mlu-cap} and \eqref{eq:capacity}, as well
as the per-demand cap \eqref{eq:demand}, all require knowing $d_i$ for
...
ratios after a feasibility-restoration step (needed only for the
throughput objective, realized by ADMM refinement and rounding in
\cref{sec:training}; under MLU the ratios are directly feasible by
\eqref{eq:conserve}). Formally, for MLU minimization
\begin{equation}
    \min_{\pi} \;
...
the link loads in \eqref{eq:sparse-mlu} are evaluated with the true
...
\eqref{eq:mlu}--\eqref{eq:nonneg}, where the same $D^t$ appears in both
...
Three solution families exist for~\eqref{eq:sparse-mlu}
and~\eqref{eq:sparse-obj}.
\emph{Solve-with-zeros} plugs $O$ directly into the
LP~\eqref{eq:mlu}--\eqref{eq:nonneg}, implicitly assuming unmeasured
...
realized MLU for~\eqref{eq:sparse-mlu}, or total satisfied demand
for~\eqref{eq:sparse-obj}. Since $R$
