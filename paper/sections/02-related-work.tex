...
runtime scales poorly with network size. Production software-defined WANs
centralize this computation to raise utilization~\cite{jain2013,hong2013,hong2018}.
Kumar \emph{et al.} proposed
...
solvers with learned models: Valadarsky \emph{et al.}~\cite{valadarsky2017}
learn edge weights from traffic history and obtain forwarding splits through
softmin routing. Perry \emph{et al.} proposed DOTE~\cite{dote}, which directly
maps historical traffic matrices to routing splits with stochastic
optimization, without explicitly predicting a future matrix. These are
predictive TE methods: historical measurements must not be confused with
knowledge of the complete target-time traffic matrix. Xu \emph{et al.} proposed
...
multipliers (ADMM)~\cite{boyd2011} fine-tuning to achieve near-optimal
allocations with
...
per-flow granularity. Graph models have also been used to predict network
behavior rather than allocate directly, as in RouteNet~\cite{rusek2019}.
To survive topology changes without retraining, AlQiam
...
large language models as general-purpose traffic planners. Full-matrix
allocation and historical-input prediction have different information
requirements. In particular, adapting DOTE or historical-input RL to
unmeasured entries requires an explicit sparse-input protocol; natural zero
traffic is not the same as an unavailable measurement. Comparisons must align
the observation mask and the latest input time, rather than give one method
current measurements while restricting another to past measurements.
...
budgets~\cite{handley2018,bhattacherjee2019,kassing2020}; simulators such as
StarPerf~\cite{lai2020} were built to quantify this time-varying behavior.
Early work routed around mobility with distributed, delay-oriented
algorithms that recompute next hops as the topology
evolves~\cite{ekici2001}. Later classical
approaches
...
satellite platforms. Even in terrestrial networks, measurement systems
trade accuracy for resources by sampling or sketching flow
records~\cite{estan2002,liu2016}, so operators routinely work with partial
counts. A classical workaround is a two-stage pipeline:
...
completion~\cite{candes2009}, or compressive sensing~\cite{zhang2009}, then
optimize routing on
...
passing under missing node features via teacher-student distillation, and You
\emph{et al.}~\cite{you2020} cast imputation itself as edge prediction on a
bipartite graph, but such
