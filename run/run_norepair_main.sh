...
run_one() {
    ...
    local csv done lock expected tag arch mode post extra logf out
    ...
    # 原始 seed 行仍完整保留，日志只显示同一条件下的聚合结果。
    {
        flock -x 9
        printf '%s,%s,%s,%s\n' "$cfg" "$rho" "$seed" "$out" >> "$csv"
        printf '%s\n' "$tag" >> "$done"
        "$PY" "$SUMMARIZER" "$csv" --expected-seeds "$expected" \
            --where "config=$cfg" --where "rho=$rho" --compact
    } 9>"$lock"
}
export -f run_one
export PY SUMMARIZER BASE TRAIN UNTRAIN LOGDIR
